...
class ROS_OM_Node:
    ...
    def execute_place(self,place, place_angle=0.0):
        ...
        joint_traj_1, time_stamps_1 = gen_place( '/root/catkin_ws/src/my_scripts_rl/recordings/dmp/home2pick.pkl',
...
def gen_place(path, place_position,position_home= np.array([0.05 ,0 ,0.17]), place_angle=0.0):
    ...
    # Home --> PlaceDown
    pose_start = gripper_orientation_pick(position_home)
    pose_start = rotate_pose_around_y(pose_start,np.deg2rad(45))
    position = place_position
    pose = gripper_orientation_pick(position)
    pose = rotate_pose_around_y(pose,place_angle)
